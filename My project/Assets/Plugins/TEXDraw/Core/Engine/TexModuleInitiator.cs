...
namespace TexDrawLib
{
    ...
    public partial class TexModuleParser
    {
        ...
        public readonly string[] overUnderVariants = new string[] {
            ...
            genericCommands = new Dictionary<string, InlineMacroAtom.InlineState>();
            {
                ...
                foreach (var item in symbolOverrides)
                {
                    (int, Atom) overrides(TexParserState state, string value, int pos)
                    {
                        ...
                        return (pos, OverridedSymbolAtom.Get(atom as SymbolAtom, item.Value));
                    };
                ...
                foreach (var item in styleVariants)
                {
                    ...
                    Action<TexParserState> styleProcessor(bool combining, bool math = false)
                    {
                    ...
                    genericCommands[item.Key] = new InlineMacroAtom.InlineState(item.Key, styleProcessor(false));
                    genericCommands["text" + item.Key] = new InlineMacroAtom.InlineState("text" + item.Key, styleProcessor(false), requiresGroupContext: true);
                    genericCommands["math" + item.Key] = new InlineMacroAtom.InlineState("math" + item.Key, styleProcessor(false, true), requiresGroupContext: true);
                    genericCommands[item.Key + styleVariantsNaming[variant]] = new InlineMacroAtom.InlineState(item.Key, styleProcessor(true));
                }
